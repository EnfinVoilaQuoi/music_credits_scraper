...
class DisabledTracksManager:
    ...
    def save_disabled_tracks(self, artist_name: str, disabled_track_ids: set[int]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Morceaux désactivés sauvegardés pour {artist_name}: {len(disabled_track_ids)} morceaux"
            )
            return True
    ...
    def load_disabled_tracks(self, artist_name: str) -> set[int]:
        ...
        try:
            ...
            if version == "2.0":
                ...
                logger.info(
                    f"Morceaux désactivés chargés pour {artist_name}: {len(disabled_track_ids)} morceaux"
                )
                ...
                logger.warning(
                    f"Fichier v1.0 détecté pour {artist_name} - migration nécessaire (impossible automatiquement)"
                )
                return set()
