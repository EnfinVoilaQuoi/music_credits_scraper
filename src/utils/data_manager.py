            ...
            with self._get_connection() as conn:
                ...
                for i, row in enumerate(rows):
                    try:
                        ...
                        try:
                            track.relationships = (
                                json.loads(relationships_raw) if relationships_raw else []
                            )
                        except (ValueError, TypeError):
                            track.relationships = []
                        ...
                        if (
                            ...
                            try:
                                ...
                                track.musical_key = key_mode_to_french_from_string(
                                    track.key, track.mode
                                )
                            except Exception as e:
                                logger.debug(
                                    f"Impossible de calculer musical_key pour track {track_id}: {e}"
                                )
                        ...
                        try:
                            ...
                            logger.debug(
                                f"JSON album_certifications invalide pour track {track_id}: {album_certifications_json!r:.100}"
                            )
                            track.album_certifications = []
