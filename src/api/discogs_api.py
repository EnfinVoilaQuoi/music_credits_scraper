...
class DiscogsClient:
    ...
    def _check_rate_limit(self):
        ...
        try:
            ...
            if hasattr(self.client, '_fetcher') and hasattr(self.client._fetcher, 'rate_limit_remaining'):
                ...
                if self.rate_limit_remaining < 5:
                    logger.warning(f"⚠️ Rate limit Discogs faible: {self.rate_limit_remaining} requêtes restantes")
    ...
    def search_track(self, track_title: str, artist_name: str, album_name: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if album_name:
                ...
                logger.info(f"🔍 Discogs: Recherche '{track_title}' de {artist_name} (album: {album_name})")
            else:
            ...
            logger.warning(f"❌ Aucune correspondance exacte trouvée sur Discogs pour '{track_title}'")
            log_api("Discogs", f"search/{track_title}", False)
            return None
    ...
    def _extract_track_from_release(self, release, track_title: str, artist_name: str) -> Optional[Dict[str, Any]]:
        """
                ...
                for credit_dict in track_data['credits']:
                    try:
                        ...
                        credit = Credit(
                            ...
                            role=role_enum,
                            role_detail=credit_dict.get('role_detail') or (
