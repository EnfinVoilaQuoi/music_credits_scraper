...
def main():
    ...
    def on_response(resp):
        if API_MARK in resp.url:
            api_events.append((time.time(), 'resp', resp.request.method, resp.url, resp.status, resp))
    ...
    try:
        ...
        if tok:
            ...
            status = ("EXPIRÉ ❌" if expired else "valide ✅") if expired is not None else "exp non lisible"
        ...
        if result:
            ...
            if elapsed > 90:
                ...
                print("🤔 Réussi ici mais échoue dans l'app : comparer les conditions"
                      " (headless, thread, session) — relancer en headless=True pour confirmer.")
        elif not yt_reqs:
            print("❌ VERDICT A : AUCUN POST /api/yt après le clic Upload.")
            print("   → Le clic ne déclenche rien : bouton/sélecteur à revoir"
            ...
            print("   → 401/403 = session/JWT ; 429 = quota ; 5xx = backend"
                  " (souvent : ingestion YouTube cassée côté site).")
        elif yt_resps:
            print(f"❌ VERDICT C (probable) : /api/yt = {yt_resps[-1][4]} mais aucune nouvelle carte parsée.")
            print(f"   Cartes matchées par _CARD_RE dans la page finale : {len(cards_now)}")
            print(f"   → comparer {stamp}_page.txt / screenshots au format attendu"
                  " « Key: X minor … BPM: NN … Camelot: NA » et ajuster _CARD_RE.")
            print(f"   corps /api/yt: {body_of(yt_resps[-1][5])[:400]}")
        else:
            print("❌ POST /api/yt parti mais AUCUNE réponse reçue avant le timeout"
