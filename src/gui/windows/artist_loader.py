...
def load_existing_artist(app):
    ...
    # Titre
    ctk.CTkLabel(dialog, text="Gestionnaire d'artistes", font=("Arial", 18, "bold")).pack(pady=15)
    ...
    ctk.CTkLabel(list_frame, text="Artistes en base de données:", font=("Arial", 14, "bold")).pack(
    ...
    if not artists_data:
        ctk.CTkLabel(
            list_frame, text="Aucun artiste dans la base de données", text_color="gray"
        ).pack(pady=20)
    ...
    for artist_info in artists_data:
        ...
        artist_button.bind(
            "<Double-Button-1>", lambda e, n=artist_info["name"]: load_on_double_click(n)
        )

        artist_widgets.append(
            {"name": artist_info["name"], "frame": artist_frame, "button": artist_button}
        )
    ...
    ctk.CTkLabel(action_frame, text="Actions:", font=("Arial", 12, "bold")).pack(
        anchor="w", padx=10, pady=(10, 5)
    )
...
"""
    ...
    # Rangée de boutons
    ctk.CTkButton(buttons_frame, text="📂 Charger", command=load_selected, width=120).pack(
        side="left", padx=8
    )
    ...
    ctk.CTkButton(buttons_frame, text="ℹ️ Détails", command=show_artist_details, width=120).pack(
        side="left", padx=8
    )

    ctk.CTkButton(buttons_frame, text="🔄 Actualiser", command=refresh_list, width=120).pack(
        side="left", padx=8
    )
