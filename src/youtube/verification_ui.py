...
class ManualVerificationInterface:
    ...
    def _setup_ui(self):
        ...
        self.candidates_tree = ttk.Treeview(
            candidates_frame, columns=columns, show="headings", height=10
        )
        ...
        scrollbar = ttk.Scrollbar(
            candidates_frame, orient=tk.VERTICAL, command=self.candidates_tree.yview
        )
        ...
        ttk.Button(buttons_frame, text="Sélectionner", command=self._select_candidate).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(buttons_frame, text="Aperçu YouTube", command=self._open_youtube).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(buttons_frame, text="Ignorer", command=self._skip_track).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(buttons_frame, text="Recherche manuelle", command=self._manual_search).pack(
            side=tk.LEFT, padx=(0, 5)
        )
    ...
    def _show_preview(self, candidate: dict):
        ...
        details.append(
            f"Chaîne: {candidate.get('channel_title', 'N/A')} ({candidate.get('channel_subscriber_count', 'N/A')} abonnés)"
        )
