...
_CAT_MAP = {
    ...
    'vidéos': 'video', 'videos': 'video', 'vidéo': 'video', 'video': 'video',
}
...
class CertMatcher:
    ...
    def __init__(self):
        ...
        logger.info(f"✅ CertMatcher : {len(self.df)} certifs unifiées "
    ...
    def _load_all(self) -> pd.DataFrame:
        ...
        if not rows:
            return pd.DataFrame(columns=[
                'artist_clean', 'title_clean', 'cat', 'level', 'date',
                'country', 'body', 'flag', 'artist_name', 'title',
                'release_date', 'publisher', 'detail_url'])
    ...
    def _load_snep(self) -> List[dict]:
        ...
        try:
            ...
            for (an, ac, ti, tc, pub, cat, lvl, rel, cdate) in cur.fetchall():
                rows.append({
                    'artist_clean': ac or '', 'title_clean': tc or '',
                    'cat': _norm_cat(cat), 'level': lvl or '',
                    'date': (str(cdate)[:10] if cdate else ''),
                    'country': 'FR', 'body': 'SNEP', 'flag': '🇫🇷',
                    'artist_name': an or '', 'title': ti or '',
                    'release_date': (str(rel)[:10] if rel else ''),
                    'publisher': pub or '', 'detail_url': '',
                })
            conn.close()
            return rows
    ...
    def _load_riaa(self) -> List[dict]:
        ...
        for _, r in df.iterrows():
            ...
            rows.append({
                ...
                'country': 'US', 'body': 'RIAA', 'flag': '🇺🇸',
                'artist_name': artist, 'title': title,
                'release_date': _to_iso_date(col(r, 'release_date')),
                'publisher': col(r, 'label'), 'detail_url': '',
            })
        return rows
    ...
    def _level_rank(self, level: str) -> float:
        ...
        if m:
            base = {'platine': 7, 'platinum': 7, 'or': 10, 'gold': 10,
                    'diamant': 4, 'diamond': 4}[m.group(2)]
            n = int(m.group(1))
    ...
    def _track_match_indices(self, a: str, t: str) -> List[int]:
        ...
        def add(sub):
            for idx in sub.index:
                if idx not in sset:
                    sset.add(idx); seen.append(idx)

        ac = df['artist_clean']; tc = df['title_clean']
        ...
        s1 = df[(ac == a) & (tc == t)]
        if s1.empty:
            s1 = df[ac.str.contains(a, regex=False, na=False) & tc.str.contains(t, regex=False, na=False)]
        add(s1)
        ...
        if m:
            ...
            if s2.empty and main:
                s2 = df[ac.str.contains(a, regex=False, na=False) & tc.str.contains(main, regex=False, na=False)]
            add(s2)

        # S3 : l'artiste apparaît en featuring (substring artiste + titre)
        s3 = df[ac.str.contains(a, regex=False, na=False) & tc.str.contains(t, regex=False, na=False)]
        add(s3)
        ...
        if not seen and len(t) >= 8:
            ...
            if not cand.empty:
                cand = cand[cand['title_clean'].apply(lambda x: isinstance(x, str) and t.startswith(x))]
                add(cand)
        return seen

    def get_track_certifications(self, artist: str, title: str,
                                 extra_artists=None) -> List[Dict[str, Any]]:
    ...
    def get_album_certifications(self, artist: str, album: str) -> List[Dict[str, Any]]:
        ...
        a = self._norm(artist); t = self._norm(album)
        df = self.df
        ...
        if exact.empty:
            exact = alb[alb['artist_clean'].str.contains(a, regex=False, na=False)
    ...
    def _format(self, rows: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for _, r in rows.iterrows():
            out.append({
                ...
                'country': r['country'], 'body': r['body'], 'flag': r['flag'],
            })
        # Tri : pays (FR, BE, US) puis niveau puis date décroissante
        order = {'FR': 0, 'BE': 1, 'US': 2}
        out.sort(key=lambda c: (order.get(c['country'], 9),
