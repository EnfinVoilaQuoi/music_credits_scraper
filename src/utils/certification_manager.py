...
class CertificationManager:
    ...
    def __init__(self, base_dir="music_credits_scraper"):
        ...
        self.paths = {
            "SNEP": {
                "csv": self.data_dir
                / "snep"
                / "certif-.csv",  # Nom du fichier téléchargé depuis SNEP
    ...
    def load_riaa_data(self):
        ...
        try:
            ...
            df["certification_date"] = pd.to_datetime(df["certification_date"]).dt.strftime(
                "%Y-%m-%d"
            )
    ...
    def update_unified_database(self):
        ...
        for body in ["SNEP", "RIAA", "BRMA"]:
            cursor.execute("SELECT COUNT(*) FROM certifications WHERE certifying_body = ?", (body,))
...
def main():
    ...
    parser.add_argument(
        "--update", choices=["all", "snep", "riaa", "brma"], help="Mettre à jour les certifications"
    )
    parser.add_argument("--query", type=str, help="Rechercher un artiste")
    parser.add_argument("--country", choices=["FR", "US", "BE"], help="Filtrer par pays")
    ...
    if args.update:
        ...
        print(
            results[
                ["artist", "title", "certification_level", "certification_date", "country_code"]
            ]
        )
