...
def start_streams_update(app):
    ...
    try:
        stored = app.data_manager.get_artist_ytm_channel(app.current_artist.id) \
            if app.current_artist else None
        if stored:
...
def run_streams_update(app, fetch_spotify: bool, fetch_ytm: bool,
                        ytm_channel_raw: str = ""):
    ...
    def run():
        try:
                    ...
                    if ytm_channel_raw:
                        ...
                        if resolved:
                            app.data_manager.set_artist_ytm_channel(
                                app.current_artist.id, resolved)
                        else:
                            logger.warning(
            ...
            app.root.after(0, lambda m=summary_msg: report.show_scrollable_report(app, 
                "Nb Streams", m))
            ...
            if _sugg:
                app.root.after(0, lambda s=_sugg, d=_kdate:
                                kworb_confirm.confirm_kworb_suggestions(app, s, d))
        except Exception as e:
