    ...
    if item:
        ...
        if tags:
            ...
            context_menu.add_command(
                ...
                command=lambda: show_track_details_by_index(app, index)
            )
            context_menu.add_command(
...
def sort_column(app, col):
    ...
    try:
        ...
        if col == "Titre":
            ...
            sort_key = lambda t: (bool(getattr(t, 'has_lyrics', False)),
...
def update_selection_count(app):
    ...
    if hasattr(app, 'selected_count_label'):
        total = len(app.current_artist.tracks) if app.current_artist and app.current_artist.tracks else 0
