...
Module PUR (aucun widget) : il vivait sous `src/gui/workers/` et a rejoint
`src/concurrency` le 2026-09-14 pour que les services hors GUI (`src/services`,
`src/cli.py`) partagent le MÊME drapeau d'arrêt que la fenêtre principale —
`src/gui/workers/lifecycle.py` n'est plus qu'un ré-export.
...
def run_worker(target, name: str | None = None) -> Future:
    ...
    def _runner(fut: Future) -> None:
        ...
        except BaseException as exc:  # noqa: BLE001 — remonté via wrap_future
...
def request_stop() -> None:
    """Lève le drapeau SANS joindre : les boucles s'arrêtent au prochain
    `stop_requested()`. C'est le geste d'un Ctrl-C en CLI — le join et
    l'annulation de la boucle viennent ensuite, par `shutdown_workers`."""
    _stop_event.set()
