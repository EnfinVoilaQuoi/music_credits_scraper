...
class TrackDetailsWindow:
    ...
    def __init__(self, app, track: Track):
        ...
        if secondary_role:
            ...
            if primary_artist:
                ctk.CTkLabel(left_column, text=f"Artiste principal: {primary_artist}").pack(anchor="w", pady=1)
            featuring_name = featured_artists or (track.artist.name if track.artist else self.app.current_artist.name)
        ...
        _sy_conf = getattr(track, 'lyrics_synced_confidence', None)
        _sy_conf_txt = {2: "2 (croisé LRCLIB+YTM)",
                        1: "1 (source unique / après départage durée)"}.get(_sy_conf, "—")
        ...
        if _has_ts and _sy_conf is not None and _sy_conf < 2:
            tech_textbox.insert("end", "  ⚠️ à vérifier (sources divergentes ou unique) — clic droit → ✏️ pour corriger\n")
        ...
        if track.last_scraped:
            tech_textbox.insert("end", f"• Dernier scraping: {helpers.format_datetime(track.last_scraped)}\n")
        if track.created_at:
            tech_textbox.insert("end", f"• Créé le: {helpers.format_datetime(track.created_at)}\n")
        if track.updated_at:
            tech_textbox.insert("end", f"• Mis à jour le: {helpers.format_datetime(track.updated_at)}\n")
        ...
        try:
            ...
            album_certs = (matcher.get_album_certifications(
                self.app.current_artist.name, track.album) if track.album else [])
                    ...
                    for body in bodies:
                        ...
                        for c in group:
                            ...
                            if c.get('release_date'):
                                txt += f"   📅 Sortie: {helpers.format_date(c.get('release_date'))}\n"
        ...
        tech_textbox.insert("end", f"• current_artist.name: {self.app.current_artist.name if self.app.current_artist else 'Non défini'}\n")
        ...
        tech_textbox.insert("end", f"• _release_date_from_api: {helpers.format_date(getattr(track, '_release_date_from_api', None))}\n")
